...
mod scalar_optimization {
	...
	fn golden_ratio_case_1_bound_method() {
		assert_eq!(bound_gr_minimize::<f64>(case_1, 4.0, -9.0, 1e-4, 2000).unwrap().round_dp(4), -3.0000)
	}
	...
	fn golden_ratio_case_2_bound_method() {
		assert_eq!(bound_gr_minimize::<f64>(case_2, 0.5, 2.0, 1e-4, 2000).unwrap().round_dp(4), 1.5000)
	}
